...
function computePretestScore(answers = {}, lang = "common") {
  try {
    // ── 新採点ロジック: 選択式20問（各1点）+ 記述式10問（各1点）= 満点30点 ──
    const { LANG_TESTS } = require("./pretestQuestions");
    const conf = LANG_TESTS[lang];
    if (!conf) {
      // 言語不明の場合は空スコアを返す
      return { score: null, total: 30, perQuestionScores: {} };
    }

    const per = {};
    let score = 0;
    const total = 30; // 選択式20 + 記述式10

    // ── Q1〜Q20: 選択式（正解ならば1点、不正解は0点）──
