...
async function computeSemiAnnualGrade(userId, employee) {
    try {
        ...
        const [attendances, goals, leaves, dailyReports] = await Promise.all([
            ...
            (async () => {
                try {
                    ...
                    return await DailyReport.find({ employeeId: employee._id, reportDate: { $gte: sixMonthsAgo } }).lean();
                } catch { return []; }
            })()
        ]);

        const noData = attendances.length === 0 && (!goals || goals.length === 0) && (!leaves || leaves.length === 0);
        if (noData) {
            return {
                grade: 'D', score: 0,
                breakdown: {
                    attendanceScore: 0, goalScore: 0, qualityScore: 0, overtimeScore: 0, leaveScore: 0,
                    sub: {
        ...
        const monthlyAvg = workCounts.length ? workCounts.reduce((s,v)=>s+v,0)/workCounts.length : 0;
        const monthlyVariance = workCounts.length > 1
            ? workCounts.reduce((s,v)=>s+Math.pow(v-monthlyAvg,2),0)/workCounts.length : 0;
        ...
        const goalsApproved  = goals ? goals.filter(g => !['draft','rejected','pending1','pending2'].includes(g.status)).length : 0;
        const goalsCompleted = goals ? goals.filter(g => g.status === 'completed' || (g.progress||0) >= 100).length : 0;
        const goalsOverdue   = goals ? goals.filter(g => g.deadline && new Date(g.deadline) < now && g.status !== 'completed').length : 0;
        const goalAvg        = goalsTotal ? Math.round(goals.reduce((s,g)=>s+(g.progress||0),0)/goalsTotal) : 0;
        // 高レベル目標（level: 'high'）の件数
        const goalsHighLevel = goals ? goals.filter(g => g.level === 'high' || g.level === '高').length : 0;
        ...
        const reportRate    = totalDays > 0 ? Math.min(1, reportCount / Math.max(1, totalDays)) : 0;
        ...
        const progressScore = goalsTotal === 0 ? 0 :
            goalAvg >= 90 ? 10 :
        ...
        const completionScore = goalsTotal === 0 ? 0 :
        ...
        const planningScore = goalsTotal === 0 ? 0 :
            goalsOverdue === 0 ? 6 :
            goalsOverdue === 1 ? 4 :
            goalsOverdue === 2 ? 2 : 0;
        ...
        // 高レベル目標が未完了なら半減
        const highCompleted  = goals ? goals.filter(g => (g.level === 'high' || g.level === '高') && (g.status === 'completed' || (g.progress||0) >= 100)).length : 0;
        const highCompRate   = goalsHighLevel > 0 ? highCompleted / goalsHighLevel : 0;
        const difficultyScore = Math.round(difficultyRaw * (0.5 + highCompRate * 0.5));
        ...
        if (punctuality < 9) actions.push({
            category: '出勤', priority: punctuality < 5 ? 'high' : 'medium',
            icon: 'fa-clock', title: '遅刻・早退をゼロにする',
            detail: `過去6か月で遅刻${lateCount}件・早退${earlyCount}件（計${lateTotal}件）。`,
            howto: '始業15分前に作業環境を整える習慣をつけましょう。3件以上は評価に大きく影響します。',
            impact: `改善で最大+${12 - punctuality}点（時間厳守）`
        });
        if (stability < 8) actions.push({
            category: '出勤', priority: stability < 4 ? 'high' : 'medium',
            icon: 'fa-calendar-check', title: '欠勤を減らす',
            detail: `過去6か月で欠勤${absentCount}日。欠勤1日で-3点、2日で-6点の大幅減点。`,
            howto: '体調不良は有給休暇を活用し、欠勤（無断・当日）を避けてください。',
            impact: `改善で最大+${10 - stability}点（安定性）`
        });
        if (consistency < 5) actions.push({
            category: '出勤', priority: 'low',
            icon: 'fa-chart-line', title: '毎月の出勤を安定させる',
        ...
        if (goalsTotal === 0) actions.push({
            category: '目標', priority: 'high',
            icon: 'fa-flag', title: '目標を登録する（最大+32点）',
            detail: '目標が1件もありません。目標管理は評価全体の32%を占める最重要項目です。',
            howto: '目標管理ページから今期の個人目標を登録してください。高レベル目標ほど加点が大きくなります。',
            impact: '目標登録・達成で最大32点加算'
        });
        else {
            if (progressScore < 7) actions.push({
                category: '目標', priority: goalAvg < 40 ? 'high' : 'medium',
                icon: 'fa-bullseye', title: '目標進捗を75%以上にする',
                detail: `現在の平均進捗は${goalAvg}%。75%未満は評価が大きく下がります。`,
                howto: '週1回以上進捗を更新し、停滞タスクは上長に相談してリスケしてください。',
                impact: `改善で最大+${10 - progressScore}点（進捗）`
            });
            if (completionScore < 7) actions.push({
                category: '目標', priority: 'medium',
                icon: 'fa-circle-check', title: '目標を完了ステータスにする',
                detail: `承認済み目標${goalsApproved}件中${goalsCompleted}件が完了（完了率${Math.round(completionRate*100)}%）。`,
                howto: '進捗100%の目標は必ず「完了」に更新してください。ステータス更新が評価に直接影響します。',
                impact: `改善で最大+${10 - completionScore}点（完了率）`
            });
            if (goalsOverdue > 0) actions.push({
                category: '目標', priority: 'medium',
                icon: 'fa-calendar-days', title: `期限超過${goalsOverdue}件を解消する`,
                detail: `期限を過ぎた未完了目標が${goalsOverdue}件あります。計画性スコアが0〜2点まで低下します。`,
                howto: '期限を現実的な日付に更新するか、達成困難な目標は上長と相談してスコープを縮小してください。',
                impact: `解消で最大+${6 - planningScore}点（計画性）`
            });
            if (difficultyScore < 3 && goalsHighLevel === 0) actions.push({
                category: '目標', priority: 'low',
                icon: 'fa-fire', title: '高レベル目標に挑戦する',
                detail: '高レベル目標（難易度：高）が0件です。難易度ボーナスが0点です。',
                howto: '目標作成時に難易度「高」を設定し、かつ達成することで最大6点のボーナスが得られます。',
                impact: '高レベル目標達成で最大+6点（難易度）'
            });
        }
        if (dailyReportScore < 5 && reportCount === 0) actions.push({
            category: '業務品質', priority: 'high',
            icon: 'fa-file-lines', title: '日報を毎日提出する',
            detail: '日報の提出記録がありません。業務品質スコアが最大半減します。',
            howto: '業務終了前に日報を提出する習慣をつけてください。提出率70%以上で+5点になります。',
            impact: '提出率90%以上で+8点（日報）'
        });
        else if (dailyReportScore < 6) actions.push({
            category: '業務品質', priority: 'medium',
            icon: 'fa-file-lines', title: '日報の提出率を90%以上にする',
        ...
        if (controlScore < 5) actions.push({
            category: '残業', priority: monthlyOT >= 40 ? 'high' : 'medium',
            icon: 'fa-moon', title: '月間残業を15時間未満に抑える',
        ...
        if (leavePending >= 2) actions.push({
            category: '休暇', priority: 'low',
            icon: 'fa-umbrella-beach', title: '未承認の休暇申請を減らす',
            detail: `${leavePending}件が未承認のままです。`,
            howto: '休暇は取得日の3営業日以上前に申請し、承認状況を定期確認してください。',
            impact: `解消で最大+${7 - leavePlanScore}点（計画的申請）`
        });

        // データ充足ペナルティの警告
        if (dataCompleteness < 1) actions.push({
            category: 'データ', priority: 'low',
            icon: 'fa-database', title: 'データ蓄積でスコア上限が上がる',
        ...
        const explanation =
            ...
            (dataCompleteness < 1 ? ` ※データ充足率${Math.round(dataCompleteness*100)}%のためスコア上限${maxScoreCap}点でキャップされています。` : '');
        ...
        return {
            ...
            raw: {
                lateCount, earlyCount, absentCount, normalCount, totalDays,
                overtimeSum, monthlyOT: Math.round(monthlyOT),
                ...
                monthCount, dataCompleteness: Math.round(dataCompleteness * 100), maxScoreCap
            },
            actions: actions.sort((a, b) => ({ high: 0, medium: 1, low: 2 }[a.priority] - { high: 0, medium: 1, low: 2 }[b.priority])),
            explanation
        };
    } catch (err) {
        ...
        return {
            grade: 'D', score: 0,
            breakdown: { attendanceScore: 0, goalScore: 0, qualityScore: 0, overtimeScore: 0, leaveScore: 0, payrollScore: 0, sub: {} },
            actions: [],
